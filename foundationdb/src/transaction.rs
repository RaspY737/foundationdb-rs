...
impl Transaction {
    ...
    pub fn get(
        ...
        .map(move |result| {
            let (bytes_count, kv_fetched, metrics) = match (&result, metrics) {
                (Ok(Some(values)), metrics) => ((lenght_key + values.len()) as u64, 1, metrics),
                (Ok(None), metrics) => (lenght_key as u64, 0, metrics),
